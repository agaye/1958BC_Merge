...
# and a high threshold of 0.90 for minimum identity by descent ('minimum_ibd'). Only twins or duplicates 
# should be found related and since 1958BC datasets do not contain twins I know they must be duplicates.
#
...
for(i in 1:(size-1)){
  ...
  for(j in (j+1):size){
    ...
    system(paste0("p-link --silent --bfile ", mergedfile," --maf 0.30 --Z-genome --min 0.90 --allow-no-sex --out ",  summaryfile))   
  }
}
